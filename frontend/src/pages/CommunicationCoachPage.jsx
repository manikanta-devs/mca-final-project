    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6 items-start">
        ...
        <div className="card space-y-2 p-3">
          <span className="text-[10px] font-bold text-slate-450 dark:text-gray-400 uppercase tracking-widest block px-3 mb-2 font-mono">Interactive Guideline Center</span>
          
          <button
            onClick={() => setActiveSection('mentor')}
            className={clsx(
              'w-full text-left px-3 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-2.5 transition-all border',
              activeSection === 'mentor'
                ? 'bg-violet-600/10 border-violet-500/20 text-violet-600 dark:text-violet-400 font-bold'
                : 'text-slate-500 dark:text-gray-500 hover:text-slate-800 dark:hover:text-gray-300 bg-transparent border-transparent'
            )}
          >
        ...
        <div className="space-y-6 select-text">
          ...
          {activeSection === 'mentor' && (
            <div className="card space-y-5">
              <div className="flex flex-col gap-1 border-b border-black/5 dark:border-gray-800 pb-4">
                <h3 className="text-md font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                  <Cpu className="w-5 h-5 text-violet-500" /> AI Career Mentor (Ask Anything)
                </h3>
                <p className="text-xs text-slate-500 dark:text-gray-550">Query your tech concepts. Sarah will formulate analogies and structured scripts automatically.</p>
              </div>
              ...
              {mentorAnswer && (
                <div className="card border-violet-500/20 space-y-4 select-text relative overflow-hidden animate-in">
                  ...
                  <div className="flex justify-between items-center pb-3 border-b border-black/5 dark:border-gray-800/80">
                    <div className="flex items-center gap-2">
                      <Cpu className="w-4 h-4 text-violet-500" />
                      <span className="text-xs font-black text-slate-900 dark:text-white font-mono">Mentor response</span>
                    </div>
                    <button
                      ...
                      className={clsx(
                        ...
                        isSpeaking
                          ...
                          : 'bg-black/5 dark:bg-white/5 border-black/5 dark:border-white/5 text-slate-700 dark:text-gray-300'
                      )}
                    >
                  ...
                  <div className="space-y-3.5 max-w-full break-words">
                    <div className="space-y-1 max-w-full break-words">
                      <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase tracking-wider block font-mono">Conceptual Definition</span>
                      <p className="text-xs text-slate-700 dark:text-gray-300 leading-relaxed font-medium break-words whitespace-pre-wrap">{mentorAnswer.definition}</p>
                    </div>

                    <div className="space-y-1 p-3.5 rounded-2xl bg-amber-500/5 border border-amber-500/25 max-w-full break-words text-slate-700 dark:text-gray-305">
                      <span className="text-[9px] font-bold text-amber-600 dark:text-amber-405 uppercase tracking-wider block font-mono">Simplified Analogy</span>
                      <p className="text-xs leading-relaxed font-medium font-sans break-words whitespace-pre-wrap">{mentorAnswer.analogy}</p>
                    </div>

                    {mentorAnswer.code && (
                      <div className="space-y-1 max-w-full overflow-hidden">
                        <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase tracking-wider block font-mono">Ideal Output Snippet</span>
                        <pre className="p-4 rounded-2xl bg-black/5 dark:bg-slate-950 text-emerald-600 dark:text-emerald-400 font-mono text-[11px] overflow-x-auto select-text border border-black/5 dark:border-white/5 leading-relaxed max-w-full break-all whitespace-pre-wrap">
                    ...
                    <div className="space-y-1.5 max-w-full break-words">
                      <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase tracking-wider block font-mono">Speaking Guidelines & Key points</span>
                      <ul className="space-y-1.5 text-xs font-semibold text-slate-650 dark:text-gray-400 max-w-full break-words">
                        {mentorAnswer.tips.map((t, i) => (
                          <li key={i} className="flex items-start gap-2 max-w-full break-words">
                            <Check className="w-3.5 h-3.5 text-emerald-650 dark:text-emerald-400 shrink-0 mt-0.5" />
          ...
          {activeSection === 'roadmap' && (
            <div className="space-y-6">
                ...
                <div className="card space-y-4">
                  ...
                  <div className="space-y-2 text-xs">
                    {(roadmap?.weaknesses || []).map(skill => (
                      <div key={skill.name} className="flex justify-between items-center p-2.5 rounded-xl bg-slate-950/5 dark:bg-slate-950/40 border border-black/5 dark:border-white/5">
                        <div>
                          <span className="font-bold text-slate-800 dark:text-gray-200 block">{skill.name}</span>
                          <span className="text-[9px] text-slate-455 dark:text-gray-400 font-normal">Mastery: {skill.mastery}</span>
                        </div>
                        <div className="text-right space-x-2">
                          <span className="px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-600 dark:text-rose-500 text-[8px] font-black uppercase">{skill.priority}</span>
                          <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">{skill.est_improvement}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="card space-y-3">
                <span className="text-[10px] font-bold text-slate-450 dark:text-gray-400 uppercase tracking-widest block font-mono">🎯 Select & Regenerate Roadmap for Topic</span>
              ...
              <div className="space-y-4">
                <span className="text-[10px] font-bold text-slate-450 dark:text-gray-400 uppercase tracking-widest block font-mono">Learning Journey Timeline</span>
                  ...
                  {(roadmap?.phases || []).map(phase => {
                    ...
                    return (
                      <div key={phase.phase_num} className="relative">
                        {/* Timeline dot */}
                        <div className={clsx(
                          'absolute -left-[31px] top-1.5 w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all z-10',
                          isCompleted
                            ? 'bg-emerald-500 border-emerald-400 text-white'
                            : isCurrent
                              ? 'bg-violet-650 border-violet-500 ring-4 ring-violet-500/20'
                              : 'bg-white dark:bg-slate-900 border-black/10 dark:border-gray-800'
                        )}>
                          {isCompleted && <Check className="w-2.5 h-2.5 stroke-[3]" />}
                        </div>

                        <div className="card space-y-3 transition-all hover:border-violet-500/20 select-text">
                          {/* Header title */}
                          <div className="flex justify-between items-center cursor-pointer" onClick={() => setExpandedPhase(isExpanded ? null : phase.phase_num)}>
                            <div>
                              <span className="text-[9px] font-bold text-violet-500 dark:text-violet-400 uppercase tracking-widest block font-mono">Phase {phase.phase_num}</span>
                              <h4 className="text-xs font-black text-slate-900 dark:text-white">{phase.title}</h4>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              <span className={clsx(
                                ...
                                isCompleted
                                  ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 border-emerald-550/10 dark:border-emerald-500/15'
                                  : isCurrent
                                    ? 'bg-violet-600/10 text-violet-650 dark:text-violet-500 border-violet-550/10 dark:border-violet-500/15'
                                    : 'bg-black/5 dark:bg-gray-800 text-slate-450 dark:text-gray-400 border-black/5 dark:border-transparent'
                              )}>
                                {phase.status}
                              </span>
                              <span className="text-[10px] font-extrabold text-slate-450 dark:text-gray-400">{phase.progress}%</span>
                            </div>
                          </div>

                          {/* Expandable card contents (Expanded Card / Section 4) */}
                          {isExpanded && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="pt-3 border-t border-black/5 dark:border-gray-800/80 space-y-4 text-xs">
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-slate-700 dark:text-gray-300 bg-slate-950/5 dark:bg-slate-950/40 p-3 rounded-2xl border border-black/5 dark:border-white/5">
                                <div>
                                  <span className="text-[9px] text-slate-450 dark:text-gray-400 uppercase block font-bold">Est. Study Time</span>
                                  <span className="font-extrabold">{phase.est_study_time}</span>
                                </div>
                                <div>
                                  <span className="text-[9px] text-slate-450 dark:text-gray-400 uppercase block font-bold">Difficulty</span>
                                  <span className="font-extrabold text-violet-500 dark:text-violet-400">{phase.difficulty}</span>
                                </div>
                                <div>
                                  <span className="text-[9px] text-slate-455 dark:text-gray-400 uppercase block font-bold">Importance</span>
                                  <span className="font-extrabold text-rose-500">{phase.importance}</span>
                                </div>
                                <div>
                                  <span className="text-[9px] text-slate-455 dark:text-gray-400 uppercase block font-bold">Concept check</span>
                                  <span className="font-extrabold text-emerald-500">STAR match</span>
                                </div>
                              </div>

                              <div className="space-y-1 select-text">
                                <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase block tracking-wider font-mono">Why this topic matters</span>
                                <p className="text-slate-650 dark:text-gray-400 leading-relaxed font-normal">{phase.why_matters}</p>
                              </div>

                              <div className="space-y-1 select-text">
                                <span className="text-[9px] font-bold text-slate-455 dark:text-gray-500 uppercase block tracking-wider font-mono">Learning outcome</span>
                                <p className="text-slate-655 dark:text-gray-400 leading-relaxed font-normal">{phase.learning_outcome}</p>
                              </div>

                              {/* Learning Pipeline (Section 4 Flow Indicators) */}
                              <div className="space-y-2">
                                <span className="text-[9px] font-bold text-slate-455 dark:text-gray-500 uppercase block tracking-wider font-mono">Concept Practice Pipeline</span>
                                  ...
                                  {(roadmap?.pipeline || []).map((step, idx) => (
                                    <React.Fragment key={idx}>
                                      <div className={clsx(
                                        ...
                                        idx <= (isCompleted ? 6 : 2)
                                          ? 'bg-violet-600/10 border-violet-500/20 text-violet-650 dark:text-violet-500'
                                          : 'bg-transparent border-black/10 dark:border-gray-200 text-slate-450 dark:text-gray-400'
                                      )}>
                                        {step}
                                      </div>
                              ...
                              {/* Free Resources List */}
                              <div className="space-y-2 border-t border-black/5 dark:border-gray-800/80 pt-3">
                                <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase block tracking-wider font-mono">Free resource links</span>
                                  ...
                                  {(phase.resources || []).map(res => {
                                    ...
                                    return (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex items-center justify-between p-3 rounded-2xl bg-black/[0.01] dark:bg-white/[0.01] hover:bg-black/[0.04] dark:hover:bg-white/[0.04] border border-black/5 dark:border-gray-800/80 hover:border-violet-500/30 transition-all font-semibold text-slate-700 dark:text-gray-200"
                                      >
                                        <div className="flex items-center gap-2">
                              ...
                              <div className="flex flex-col sm:flex-row gap-3 pt-3 border-t border-black/5 dark:border-gray-800/80">
                                <button
                                  onClick={() => navigate('/dashboard/quiz', { state: { topic: phase.title, difficulty: phase.difficulty } })}
                                  className="flex-1 py-2.5 bg-gradient-to-r from-orange-600/10 to-amber-600/10 hover:from-orange-600/20 hover:to-amber-600/20 border border-orange-500/20 hover:border-orange-500/35 text-orange-600 dark:text-orange-400 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5"
                                >
                                ...
                                <button
                                  ...
                                  className="flex-1 py-2.5 bg-gradient-to-r from-emerald-600/10 to-teal-600/10 hover:from-emerald-600/20 hover:to-teal-600/20 border border-emerald-500/20 hover:border-emerald-500/35 text-emerald-600 dark:text-emerald-400 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5"
                                >
              ...
              <div className="card bg-gradient-to-br from-slate-950 to-slate-900 text-white border-none shadow-xl relative overflow-hidden p-6 rounded-3xl">
                ...
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-center">
                  <div className="lg:col-span-2 space-y-2">
                    <span className="text-[9px] text-violet-300 font-bold uppercase tracking-wider block">Overall Roadmap Progress</span>
          ...
          {activeSection === 'guides' && (
            <div className="card space-y-6">
              <div className="flex flex-col gap-1 border-b border-black/5 dark:border-gray-800 pb-4">
                <h3 className="text-md font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-cyan-600 dark:text-cyan-405" /> HR & Project Mentor Guides
                </h3>
                <p className="text-xs text-slate-500 dark:text-gray-500">Structured scripts, key components, and explanations of *why* responses work.</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {MENTOR_GUIDES.map((g) => (
                  <div 
                    key={g.id} 
                    className="p-5 rounded-2xl bg-black/[0.01] dark:bg-white/[0.01] border border-black/5 dark:border-gray-800 hover:border-violet-500/20 transition-all flex flex-col gap-4 select-text"
                  >
                    <div className="flex justify-between items-start">
                      <div className="space-y-0.5">
                        <span className="px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/15 text-[8px] font-black uppercase tracking-wider">
                          {g.category}
                        </span>
                    ...
                    <div className="space-y-1">
                      <span className="text-[9px] font-bold text-slate-450 dark:text-gray-400 uppercase tracking-widest block font-mono">Why it works</span>
                      <p className="text-[11px] text-slate-550 dark:text-gray-500 leading-relaxed font-normal">{g.why}</p>
                    </div>

                    <div className="space-y-1.5">
                      <span className="text-[9px] font-bold text-slate-455 dark:text-gray-400 uppercase tracking-widest block font-mono">Structural Blueprint</span>
                      <div className="p-2 rounded-xl bg-black/5 dark:bg-slate-950 border border-black/5 dark:border-white/5 text-[10px] text-slate-700 dark:text-gray-300 font-mono text-center">
                        {g.blueprint}
                      </div>
                    </div>

                    <div className="p-3.5 rounded-xl bg-black/[0.02] dark:bg-slate-950/50 border border-black/5 dark:border-white/5">
                      <span className="text-[9px] font-black text-rose-500 dark:text-rose-400 uppercase tracking-widest block mb-1 font-mono">Model Wording Script</span>
                      <p className="text-[11px] text-slate-650 dark:text-gray-305 leading-relaxed italic font-medium">&quot;{g.template}&quot;</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeSection === 'sandbox' && (
            <div className="card space-y-6">
              <div className="flex flex-col gap-1 border-b border-black/5 dark:border-gray-800 pb-4">
                ...
                <p className="text-xs text-slate-550 dark:text-gray-500">Speak aloud to analyze pacing (WPM), track filler word counts, and receive a complete AI critique.</p>
              </div>

              {/* Prompt selection */}
              <div className="flex flex-col gap-2 p-4 rounded-2xl bg-black/[0.02] dark:bg-white/[0.02] border border-black/5 dark:border-white/5">
                <span className="text-[10px] font-bold text-slate-450 dark:text-gray-400 uppercase tracking-wider font-mono">Select Practice Prompt</span>
                <select
                  value={sandboxPrompt}
              ...
              <div className="grid grid-cols-1 md:grid-cols-[1.5fr_0.5fr] gap-4 items-center bg-black/5 dark:bg-slate-950 p-5 rounded-3xl border border-black/5 dark:border-white/5 relative overflow-hidden">
                <div className="space-y-4">
                  {/* Visualizer canvas */}
                  <div className="w-full h-16 bg-white dark:bg-slate-950/80 rounded-2xl relative overflow-hidden flex items-center justify-center border border-black/5 dark:border-white/5">
                    <canvas ref={sandboxCanvasRef} width="400" height="64" className="w-full h-full" />
                    {!sandboxActive && (
                      <div className="absolute text-xs text-slate-450 dark:text-gray-550 font-bold uppercase tracking-wider pointer-events-none font-mono">
                        Visualizer Idle
                      </div>
                    )}
                  </div>

                  <div className="flex gap-6 text-xs font-semibold text-slate-800 dark:text-white">
                    <div>
                      <span className="text-[9px] text-slate-450 dark:text-gray-500 uppercase block font-mono">Speech Pacing</span>
                      <span className="text-lg font-black text-cyan-600 dark:text-cyan-400">{sandboxWpm} WPM</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-450 dark:text-gray-500 uppercase block font-mono">Filler Words</span>
                      <span className="text-lg font-black text-rose-600 dark:text-rose-500">{sandboxFillers} detected</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-450 dark:text-gray-500 uppercase block font-mono">Recording Time</span>
                      <span className="text-lg font-black text-violet-650 dark:text-violet-400">{sandboxSecondsRef.current}s</span>
                    </div>
                  </div>
                </div>
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      sandboxActive
                        ...
                        : 'bg-violet-600 hover:bg-violet-550 border-violet-400 shadow-lg shadow-violet-600/30'
                    )}
                  >
              ...
              {sandboxTranscript && (
                <div className="space-y-4 animate-in select-text">
                  <div className="space-y-1.5">
                    <span className="text-[9px] font-bold text-slate-450 dark:text-gray-500 uppercase tracking-widest block font-mono">Live Transcript</span>
                    <div className="p-4 rounded-2xl bg-black/[0.02] dark:bg-gray-800/40 border border-black/5 dark:border-white/5 text-xs font-semibold text-slate-800 dark:text-gray-300 leading-relaxed select-text">
                      {sandboxTranscript}
              ...
              {sandboxCritique && (
                <div className="card border-violet-500/20 space-y-4 animate-in relative overflow-hidden select-text">
                  ...
                  <div className="border-b border-black/5 dark:border-gray-800 pb-3">
                    <span className="text-[9px] font-bold text-violet-500 dark:text-violet-400 uppercase tracking-widest block font-mono">Speech critique</span>
                    <h4 className="text-xs font-black text-slate-900 dark:text-white mt-0.5">{sandboxCritique.headline}</h4>
                  </div>

                  <div className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold">
                      <div className="space-y-1 bg-black/[0.02] dark:bg-gray-800/35 p-3 rounded-xl border border-black/5 dark:border-white/5">
                        <span className="text-[9px] font-bold text-cyan-600 dark:text-cyan-400 uppercase tracking-wider block font-mono">Pacing check</span>
                        <p className="text-slate-650 dark:text-gray-300 leading-relaxed font-sans">{sandboxCritique.pacing_critique}</p>
                      </div>
                      <div className="space-y-1 bg-black/[0.02] dark:bg-gray-800/35 p-3 rounded-xl border border-black/5 dark:border-white/5">
                        <span className="text-[9px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider block font-mono">Filler words check</span>
                        <p className="text-slate-650 dark:text-gray-300 leading-relaxed font-sans">{sandboxCritique.fillers_critique}</p>
                      </div>
                    </div>

                    <div className="space-y-1 select-text">
                      <span className="text-[9px] font-bold text-slate-455 dark:text-gray-500 uppercase tracking-wider block font-mono">Structural delivery</span>
                      <p className="text-xs text-slate-700 dark:text-gray-300 leading-relaxed font-sans">{sandboxCritique.structural_critique}</p>
                    </div>

                    <div className="space-y-2">
                      <span className="text-[9px] font-bold text-slate-455 dark:text-gray-500 uppercase tracking-wider block font-mono">Spoken technical keywords detected</span>
                      <div className="flex gap-2 flex-wrap">
                        {sandboxCritique.keywords_used.map(word => (
                          <span key={word} className="px-2.5 py-1 rounded bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 text-[10px] text-slate-500 dark:text-gray-400 font-bold">
                            {word}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="space-y-1.5 border-t border-black/5 dark:border-white/5 pt-3">
                      <span className="text-[9px] font-bold text-slate-455 dark:text-gray-500 uppercase tracking-wider block font-mono">Actionable tips for next attempt</span>
                      <ul className="space-y-1.5 text-xs font-semibold text-slate-650 dark:text-gray-400">
